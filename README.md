...
| **Ravi job apply** | Remote Tosca / SAP QA applications | **Ravi Kumar** | `ravik021995@gmail.com` (Reply-To often `ravirajkumar712@gmail.com`) | Tosca / SAP S/4 / Playwright. **No Teradata DBA** |
...
This repo **does not** log into Indeed, Naukri, Workday, or LinkedIn and click Apply. It emails recruiter addresses (and can submit Greenhouse / Lever when the kit can). Roles without an inbox stay in a queued CSV for manual Easy Apply.
...
1. Harvest new Tosca / SAP / automation jobs into the worldwide Excel.
...
4. If `--mail` is set: email **new** unused recruiter inboxes for Ravi (remote Tosca/SAP) and Jaya (remote Teradata-family).
5. Refresh `dashboard/watch.js` and append `data/watch_jobs.log`.

Each application email attaches **only** the JD-built resume from job-apply-kit (`create_packet`), not the static PDF/docx sitting in the profile. For Ravi, headline, summary, skills order, and bullets are rebuilt from **that posting’s JD** every send (truth-only facts from `profile.yaml`).
...
| Script | Purpose |
|---|---|
| `watch_jobs.py` | Hourly cycle: harvest + Excel + apply mail for Ravi and Jaya. Installs hidden Windows task `SAPDeskWatchJobs`. |
...
| `apply_remote_ravi.py` | Email Ravi to remote Tosca/SAP rows that have a recruiter inbox. |
...
### Ravi Kumar — Tosca / SAP QA

- Kit user: `ravi`
- Track: `automation`
- Source file: newest of `data/jobs_worldwide.csv` or `data/jobs_worldwide.updated.csv`
- Filter: remote + (Tosca/Tricentis **or** SAP in title/JD). Search-term alone is not enough.
- Skip: intern / Praktikum (unless Tosca), food company “Tosca”, ADA inboxes (`accessibility@`, `accommodation@`)
...
**Worldwide (SAP + Tosca / automation)**
...
| `--focus automation` | Tosca, Tricentis, SAP test automation, QA automation |
...
```powershell
# one cycle now (harvest + Excel + mail)
...
## GitHub Pages + Run now (no Cursor)
...
4. Keep the runner process (or Windows service) running while you want “Run now” / hourly Actions to work.

The workflow `Watch jobs` (`.github/workflows/watch-jobs.yml`) runs:

```text
python -u scripts\watch_jobs.py --once --mail
```

inside `C:\Users\ravir\ConsultingCompanyAutomation` (override via the `local_repo` dispatch input). It then copies `dashboard/` → `docs/` and pushes so Pages updates.

Ravi and Jaya mailboxes stay separate (same scripts as the local schtask `SAPDeskWatchJobs`).

### 3. Click Run now on the site
...
5. **Refresh run status** polls recent runs. After the runner finishes and pushes, reload the page to see new `watch.js` counts.

**No-token fallback:** use the **Open Actions** link → **Run workflow**.
...
| `data/ravi_remote_apply_queued.csv` | Remote Tosca/SAP with no sendable inbox |
...
- No mixing of Ravi Tosca facts onto Jaya packets, or Teradata DBA facts onto Ravi packets.
